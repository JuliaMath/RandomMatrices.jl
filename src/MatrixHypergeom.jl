...
function mhg{T1<:Real, T2<:Real, T3<:Real, T4<:Real}(MAX::Integer, k::Integer, alpha::Real, p::Vector{T1}, q::Vector{T2}, x::Vector{T3}, y::Vector{T4})
    ...
    for i=1:n
        Sx[n+i]=1
        xn[(MAX+2)*i+2]=1
        for j=2:MAX+1 xn[(MAX+2)*i+j+1]=xn[(MAX+2)*i+j]*x[i] end
    end
    ...
    while (h>0)
        if ((l[h+1]<l[h]) && (MAX>=sl) && (z[h+1]!=0)) 
            ...
            if (h<n) 
                ...
                println("h<n cc=",cc," prodx=",prodx[h+1]," Sx",Sx[nmu*n+h]," nmu=",nmu," n=",n," h=",h)
                ...
                for i=1:nhstrip
                    ...
                    blm[j+1]*=zn/dn
                    @printf("~blm=%f %f %f %f\n",blm[1],blm[2],blm[3],blm[4])

                    @printf("~blm zn=%f dn=%f\n",zn,dn)
                    mu[gz+1]-=1
                    mt[gz+1]-=alpha
                    slm+=1
                    ...
                    for k=h+1:n
                        ...
                        println("!!k =",k," blm=",blm[j+1]," Sx",Sx[nmu*n+k-1]," xn=",xn[k*(MAX+2)+slm+1])
                    end
                    if y!=[]
                        for k=h+1:n
                ...
                if y!=[]
                    ...
                    println("sl=",sl," z=",z[h+1], "Sx=",Sx[w*n+n])
                end
            else #h>=n
                Sxq=deepcopy(Sx)
...
#println(mhg(20,2,[],[],[1]))
#println(exp(1))
#println(mhg(20,2,[],[],[1,1]))
#println(exp(2))
println(mhg(20,2,[],[],[1,1,1]))
println(exp(3))
#println(mhg(20,2,[],[],[1,1,1,1]))
#println(exp(4))
